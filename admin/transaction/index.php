        ...
        <div class="container-fluid">
			<table class="table table-bordered table-stripped">
                    ...
                    <tbody>
                        <?php 
                        $i = 1;
                        $qry = $conn->query("SELECT t.*,c.name as service,concat(lastname,', ', firstname,' ',middlename) as user from `transaction_list` t inner join service_list c on t.service_id = c.id inner join users u on t.user_id = u.id order by unix_timestamp(t.`date_created`) desc ");
